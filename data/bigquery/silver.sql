 CREATE TABLE IF NOT EXISTS `silent-blade-487608-u0.silver_dataset.departments` (
    dept_id STRING,
    name STRING,
...
TRUNCATE TABLE `silent-blade-487608-u0.silver_dataset.departments`;

-- 3. full load 
INSERT INTO `silent-blade-487608-u0.silver_dataset.departments`
SELECT DISTINCT 
    deptid,
    name,
    CASE 
...
FROM (
    SELECT DISTINCT *  FROM `silent-blade-487608-u0.bronze_dataset.departments`
);

-------------------------------------------------------------------------------------------------------

-- 1. Create table providers 
CREATE TABLE IF NOT EXISTS `silent-blade-487608-u0.silver_dataset.providers` (
    ProviderID STRING,
    FirstName STRING,
    LastName STRING,
...
-- 2. Truncate Silver Table Before Inserting 
TRUNCATE TABLE `silent-blade-487608-u0.silver_dataset.providers`;

-- 3. full load 
INSERT INTO `silent-blade-487608-u0.silver_dataset.providers`
SELECT DISTINCT 
    ProviderID,
    FirstName,
    LastName,
    ...
    END AS is_quarantined
FROM (
    SELECT DISTINCT * FROM `silent-blade-487608-u0.bronze_dataset.providers`
 );
...
--------------------------------------------------
-- encounters table
CREATE TABLE IF NOT EXISTS `silent-blade-487608-u0.silver_dataset.encounters` (
  EncounterID STRING,
  PatientID STRING,
  EncounterDate DATE,
...
TRUNCATE TABLE `silent-blade-487608-u0.silver_dataset.encounters`;
INSERT INTO `silent-blade-487608-u0.silver_dataset.encounters`
SELECT DISTINCT 
  EncounterID,
  PatientID,
  ...
  END AS is_quarantined
FROM (
  SELECT DISTINCT * FROM `silent-blade-487608-u0.bronze_dataset.encounters`
);

--------------------------------------
CREATE TABLE IF NOT EXISTS `silent-blade-487608-u0.silver_dataset.transactions` (
...
TRUNCATE TABLE `silent-blade-487608-u0.silver_dataset.transactions`;
INSERT INTO `silent-blade-487608-u0.silver_dataset.transactions`
SELECT DISTINCT 
  TransactionID,
  EncounterID,
  PatientID,
  ...
  END AS is_quarantined
FROM (
  SELECT DISTINCT * FROM `silent-blade-487608-u0.bronze_dataset.transactions`
);

-----------------------------
-- 1. Create 
CREATE TABLE IF NOT EXISTS
`silent-blade-487608-u0.silver_dataset.claims` (
    claim_id STRING,
...
SELECT
    ...
    END AS is_quarantined
FROM
    `silent-blade-487608-u0.bronze_dataset.claims` t1;
